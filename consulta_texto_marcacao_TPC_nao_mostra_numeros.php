...
$controlo=$cookie_value;
	...
	$id_do_TPC_para_miudos=obterfile_id_de_TPC_para_marcarditados ($controlo);
...
#ERRO $id_do_TPC_para_miudos
	...
	#inserir_em_dic2me_controlo_id_do_tpc_mostrar_miudos_id_do_TPC($controlo, $id_linha);
...
#inserir_em_dic2me_controlo_id_do_tpc_mostrar_miudos_id_do_TPC($controlo, $id_linha);
$quantos=garante_obter_unico_obter_id_da_tabela_dic2me_controlo_id_do_tpc_mostrar_miudos ($controlo,$id_linha,$id_do_TPC_para_miudos);
#garante_obter_unico_obter_id_da_tabela_dic2me_controlo_id_do_tpc_mostrar_miudos ($controlo,$id_a_validar,$id_mostrar_criancas)
 #$quantos=garante_obter_unico_obter_id_da_tabela_dic2me_controlo_id_do_tpc_mostrar_miudos($controlo, $id_linha,$id_do_TPC_para_miudos);
 $protege=proteger_marcacao_do_mesmo_ditado($controlo, $id_linha, $id_do_TPC_para_miudos);
 ...
 do {
	 $mais="sim";
			 ...
			 ?><br><?php
			 echo $quantos;
			if (($quantos > 0) || ($protege == 10))
			  {
				  echo "protege_dentrodo_if";
				  echo $protege;
				  ?><br><?php
    $quantos=garante_obter_unico_obter_id_da_tabela_dic2me_controlo_id_do_tpc_mostrar_miudos($id_a_mostrar_alunos_do_tpc);
$id_a_mostrar_alunos_do_tpc++;
$protege++;
 	...
 	if (($quantos == 0) && ($id_texto != "") && ($id_do_TPC_para_miudos != ""))
	{
		$id_do_TPC_para_miudos=gerir_dic2me_controlo_id_do_tpc_mostrar_miudos($controlo, $id_linha);
...
$id_linha=$_POST['texto'];	
	...
	if ( $quantos_TPCmarcados >= $total_alunos)
	{
		$criou_bem="SIM";
	}
...
#processo para inserir referenciador para futuros alunos dessa turma importante 0999888
...
echo "hoje já marcou esse ditado, não pode marcar no mesmo dia para a mesma turma o mesmo ditado duas vezes";
...
?>
 ...
 if (		$criou_bem == "SIM" )
  ...
  <h1> <?php echo $id_a_mostrar_alunos_do_tpc ?></h1>
  <br>
  <p>
  <br>
  <p>
