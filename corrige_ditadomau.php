...
for ($j = 0; $j <= $tamanho_escrito-1; $j++){
  $ditadolimpo = trim($ditado[$j], "\0");
            ...
            $outros_ditado = explode(".",  $ditadolimpo);
$valida_ditado=count($outros_ditado);

            
            $textolimpo = trim($texto[$j], "\0");
$outros_texto = explode(".", $textolimpo);
$valida_texto=count($outros_texto);


if ( $valida_ditado > 0)
{
$palavra =$outros_ditado[0];
}
...
if ( $valida_texto == 0)
{
$palavra_texto =$textolimpo;
}

if ( $valida_texto > 0)
{
$palavra_texto = $outros_texto[0];
}
...
$palavra=$str7;
 ...
 if (strcmp($palavra_texto, $palavra) !== 0)
{

$palavra_correcta[$c]=$textolimpo;
$palavra_errada[$c]=$palavra;
$palavra_correcta[$c]=$palavra_texto;
$palavra_errada[$c]=$palavra;
...
//Comparara se é diferente do texto apenas, se for só isso não é erro.
...
?>
 <font color="blue"><?php  echo  $palavra_correcta[$i]; ?> </font>
 <?php } ?> <?php
}
